...
:)
import module namespace p="de/bottlecaps/railroad/convert/xq/parser.xq";
...
-->
        ...
        <form name="data" method="POST" enctype="multipart/form-data" onsubmit="return false;">
          ...
          <script type="text/javascript">var {$ui:EBNF}    = 4;</script>
          <script type="text/javascript">var {$ui:OPTIONS} = 5;</script>
          ...
          <ul id="tabs">
           ...
           ><li><a name="tab" href="javascript:tab({$ui:EBNF})">View EBNF</a>&#32;</li
...
{
  let $ast := p:parse($ebnf)
  return
    if ($ast instance of element(g:grammar)) then
      let $options := ui:options()
      let $grammar :=
        t:transform
        (
      ...
      {
        "error:"[not($ast instance of element(ERRORS))],
        data($ast)
      }
};
